...
import { z } from 'zod';
import type { PricingManifest, RuntimeBrowserTimePricing, RuntimeFlatPricing, RuntimeMeteredPricing } from './pricing.js';
...
const browserTimeSchema = z.object({
  kind: z.literal('metered'), unit: z.literal('browser-time'),
  provider: z.literal('cloudflare-browser-rendering'),
  browserUsdMicrosPerHour: z.literal(90_000), baseCostMicros: z.literal(200),
  maximumBrowserMs: z.number().int().positive(), minimumCredits: z.literal(1),
  reserveCredits: z.number().int().positive(), targetMarginBps: z.literal(8_000),
}).strict();
const manifestSchema = z.object({
  version: z.literal(1), creditUsdMicros: z.literal(1_000),
  routes: z.record(z.union([freeSchema, flatSchema, meteredSchema, browserTimeSchema])),
}).strict();
...
export function meteredPrice(manifest: PricingManifest, route: string): RuntimeMeteredPricing {
  const price = manifest.routes[route];
  if (price?.kind !== 'metered' || price.unit !== 'provider-cost') {
    throw new Error(`missing provider-cost pricing for ${route}`);
  }
  return price;
}

export function browserTimePrice(manifest: PricingManifest, route: string): RuntimeBrowserTimePricing {
  const price = manifest.routes[route];
  if (price?.kind !== 'metered' || price.unit !== 'browser-time') {
    throw new Error(`missing browser-time pricing for ${route}`);
  }
  return price;
}
